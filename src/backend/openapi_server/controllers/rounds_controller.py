...
def rounds_rid_first_card_cid_get(rid: str) -> Tuple[RoundsRidFirstCardCidGet200Response, int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_first_card_cid_get called with rid={rid}")
        ...
        logger.debug(f"Found first card CID: {cid}")
        return RoundsRidFirstCardCidGet200Response(cid=cid), 200
    except Exception as e:
        logger.exception(f"Error in rounds_rid_first_card_cid_get: {e}")
...
def rounds_rid_first_card_get(rid: str) -> Tuple[Union[ApiJasskarte, None], int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_first_card_get called with rid={rid}")
        resp = supabase.table('plays').select('CID').eq('RID', rid).limit(1).maybe_single().execute()
        cid = resp.data.get('CID') if resp and resp.data else None
        logger.debug(f"First card CID: {cid}")
        if not cid:
            logger.info("No first card found, returning None")
            return None, 200
        ...
        logger.debug(f"Card data retrieved: {d}")
        ...
        logger.exception(f"Error in rounds_rid_first_card_get: {e}")
        return None, 500
...
def rounds_rid_played_cards_get(rid: str) -> Tuple[List[ApiJasskarte], int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_played_cards_get called with rid={rid}")
        ...
        logger.exception(f"Error in rounds_rid_played_cards_get: {e}")
        return [], 500
...
def rounds_rid_plays_post(rid: str, body: Dict) -> Tuple[None, int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_plays_post called with rid={rid}")
        body_data = connexion.request.get_json()
        logger.debug(f"Request body: {body_data}")
        req = AddPlayRequest.from_dict(body_data)
        supabase.table('plays').insert({'RID': rid, 'UID': req.uid, 'CID': req.cid}).execute()
        logger.info(f"Inserted play: UID={req.uid}, CID={req.cid} for RID={rid}")
        return None, 200
    except Exception as e:
        logger.exception(f"Error in rounds_rid_plays_post: {e}")
        return None, 500
...
def rounds_rid_turn_get(rid: str) -> Tuple[GamesGidNextPlayerUidGet200Response, int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_turn_get called with rid={rid}")
        ...
        logger.debug(f"Current turn UID: {uid}")
        return GamesGidNextPlayerUidGet200Response(uid=uid), 200
    except Exception as e:
        logger.exception(f"Error in rounds_rid_turn_get: {e}")
...
def rounds_rid_turn_put(rid: str, body: Dict) -> Tuple[None, int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_turn_put called with rid={rid}")
        body_data = connexion.request.get_json()
        logger.debug(f"Request body: {body_data}")
        req = UpdateTurnRequest.from_dict(body_data)
        supabase.table('rounds').update({'whoIsAtTurn': req.uid}).eq('RID', rid).execute()
        logger.info(f"Updated turn to UID={req.uid} for RID={rid}")
        return None, 200
    except Exception as e:
        logger.exception(f"Error in rounds_rid_turn_put: {e}")
        return None, 500
...
def rounds_rid_winner_put(rid: str, body: Dict) -> Tuple[None, int]:  # noqa: E501
    try:
        logger.info(f"rounds_rid_winner_put called with rid={rid}")
        body_data = connexion.request.get_json()
        logger.debug(f"Request body: {body_data}")
        req = UpdateWinnerRequest.from_dict(body_data)
        supabase.table('rounds').update({'winnerid': req.winner_uid}).eq('RID', rid).execute()
        logger.info(f"Updated winner to UID={req.winner_uid} for RID={rid}")
        return None, 200
    except Exception as e:
        logger.exception(f"Error in rounds_rid_winner_put: {e}")
        return None, 500
